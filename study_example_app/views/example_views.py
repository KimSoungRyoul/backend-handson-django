...
def user_create_api(request):
    # 회원을 생성하는 코드 개발자가 직접 구현
    return 200, {"detail": "회원가입이 완료됐습니다."}


@ninja_api.get("/user")
def user_list_api(request):
    # 회원 목록을 조회하는 코드 개발자가 직접구현
    return 200, list(User.objects.all())


@ninja_api.patch("user/{pk}")
def user_partial_update_api(request, pk: int):
    # 회원 정보를 수정하는 코드 개발자가 직접구현
    return 200, {"detail": "회원 정보 수정이 완료됐습니다."}

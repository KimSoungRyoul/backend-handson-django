...
from study_example_app.models.fields import MaskingField
...
class ExampleUser(models.Model):
    ...
    a_model = ForeignKey(to="AModel", on_delete=models.CASCADE, null=True)


class AModel(Model):
    a_field = CharField(max_length=32)
    b_set = ManyToManyField(to="Bmodel", through="AModelBModelRelation")
    a_masking_field = MaskingField(max_length=32, default="aa")


class AModelBModelRelation(Model):
    a = ForeignKey(to="AModel", on_delete=CASCADE)
    b = ForeignKey(to="Bmodel", on_delete=CASCADE)


class Bmodel(Model):
    b_field = CharField(max_length=16)
    b_int_field = IntegerField(default=3)
    b_masking_field = MaskingField(max_length=32)
...
class Consumer(models.Model):
    ...
    product_set = GenericRelation(to="Album")
...
class Menu(models.Model):
    ...
    restaurant = models.ForeignKey(to="Restaurant2", on_delete=models.CASCADE, help_text="이 메뉴를 판매하는 음식정")
...
class Album(Product):
    a_field = CharField(max_length=16)
    a_int_field = IntegerField(default=3)
    a_masking_field = MaskingField(max_length=32)

    # object_type = models.CharField(choices=Consumer.ProductType.choices, default=Consumer.ProductType.ALBUM)
...
class Book(Product):
    ...
    b_int_field = IntegerField(default=3)
    b_masking_field = MaskingField(max_length=32)
    tags = GenericRelation(to="Consumer")
    # object_type = models.CharField(choices=Consumer.ProductType.choices, default=Consumer.ProductType.ALBUM)
...
class Movie2(Product):
    m_field = CharField(max_length=16)
    m_int_field = IntegerField(default=3)
    m_masking_field = MaskingField(max_length=32)
    tags = GenericRelation(to="Consumer")
    # object_type = models.CharField(choices=Consumer.ProductType.choices, default=Consumer.ProductType.ALBUM)
    class Meta:
        db_table = "movie22"

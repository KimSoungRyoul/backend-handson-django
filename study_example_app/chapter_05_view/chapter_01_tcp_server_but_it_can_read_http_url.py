...
while True:
    ...
    if http_path == '/hello':
        response_body = '{"message" : "안녕 나도 반가워 난 TCP/IP 서버야"}'.encode('utf-8')
        response_body_length = len(response_body)
    elif http_path == '/bye':
        response_body = '{"message" : "그래 잘가 나는 TCP/IP 서버야"}'.encode('utf-8')
        response_body_length = len(response_body)
    else:
        response_body = '{"message" : "TCP/IP 서버야"}'.encode('utf-8')
        response_body_length = len(response_body)
